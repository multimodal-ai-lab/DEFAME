...
class ObjectDetector:
    ...
    def recognize_objects(self, image: Image.Image) -> List[str]:
        """
...
class GeoLocator:
    ...
    def locate(self, image: Image.Image, choices: List[str] = None, top_k=10) -> Dict[str, float]:
        """
        Perform geolocation on an image.
        
        :param image: A PIL image.
        :param choices: A list of location choices. If None, uses a default list of countries.
        :return: A dictionary of location predictions and their probabilities.
        """
        if choices is None:
        ...
        top_locations = dict(sorted(confidences.items(), key=lambda x: x[1], reverse=True)[:top_k])
...
class Actor:
    ...
    def _perform_object_recognition(self, action: ObjectRecognition) -> list[Result]:
        ...
        objects = self.object_detector.recognize_objects(action.image)
        return [Result(f"Objects recognized: {objects}")]

    def _perform_reverse_search(self, action: ReverseSearch) -> list[Result]:
        self.logger.log(f"Performing reverse image search.")
        similar_images = self.reverse_searcher.search(action.image)
        return [Result(f"Similar images found: {similar_images}")]
    
    def _perform_geo_location(self, action: GeoLocation) -> list[Result]:
        self.logger.log(f"Performing geolocation on the image.")
        top_k_locations = self.geo_locator.locate(action.image, self.top_k)
        return [Result(f"The most likely countries where the image was taken are: {top_k_locations}")]
    
    def _perform_face_recognition(self, action: FaceRecognition) -> list[Result]:
        self.logger.log(f"Performing face recognition on the image.")
        recognized_faces = self.face_recognizer.recognize_faces(action.image)
        return [Result(f"Faces recognized: {recognized_faces}")]
    ...
    def _perform_source_credibility_check(self, action: SourceCredibilityCheck) -> list[Result]:
        ...
        credibility_score = self.source_checker.check_credibility(action.source)
        return [Result(f"Source credibility score: {credibility_score}")]
    
    def _perform_ocr(self, action: OCR) -> list[Result]:
        self.logger.log(f"Performing OCR on the image.")
        extracted_text = self.image_reader.extract_text(action.image)
        return [Result(f"Extracted text: {extracted_text}")]
