...
from safe import get_atomic_facts, rate_atomic_fact
from safe.decontextualize import revise_fact
from safe.check_worthiness import determine_check_worthiness
...
def verify_claim(
    ...
    rate_data, _ = rate_atomic_fact.check_atomic_fact(
        atomic_fact=claim, rater=rater
    )
    return rate_data.answer, rate_data.response
...
def main(content: str, rater: modeling.Model):
    print(f"Content to be fact-checked: '{content}'")

    print("Decomposing...")
    atomic_facts = get_atomic_facts.main(response=content, model=rater)

    print("Decontextualizing...")
    atomic_facts_decontextualized = decontextualize(atomic_facts, content, rater)
    for atomic_fact in atomic_facts_decontextualized:
        print(f"'{atomic_fact}'")

    print("Filtering for check-worthy claims...")
    check_worthiness_ratings = determine_check_worthiness(atomic_facts_decontextualized, content, rater)
    print(check_worthiness_ratings)

    print("Verifying the claims...")
    veracities = []
    justifications = []
    for claim, check_worthy in zip(atomic_facts_decontextualized, check_worthiness_ratings):
        veracity, justification = verify_claim(claim, rater)
        veracities.append(veracity)
        justifications.append(justification)

    for claim, veracity, justification in zip(atomic_facts_decontextualized, veracities, justifications):
        print(f"The claim '{claim}' is {veracity}.")
        print(justification)
        print()


def decontextualize(atomic_facts: list[str], context: str, model):
    decontextualized_facts = []
    for atomic_fact in atomic_facts:
        _, decontextualized_fact = revise_fact(context, atomic_fact, model)
        decontextualized_facts.append(decontextualized_fact)
    return decontextualized_facts
